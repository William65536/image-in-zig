...
fn verifyWriter(comptime Writer: type) void {
    const message = "Image writer must be a structure with a write member function and an error set called Error.";

    switch (@typeInfo(Writer)) {
        .Struct => {},
        else => @compileError(message ++ " Cannot use " ++ @typeName(Writer) ++ " because it is not a structure."),
    }

    if (!@hasDecl(Writer, "write")) {
        @compileError(message ++ " Cannot use " ++ @typeName(Writer) ++ " because it does not contain a write member function.");
    }

    if (!@hasDecl(Writer, "Error")) {
        @compileError(message ++ " Cannot use " ++ @typeName(Writer) ++ " because it does not contain an error set called Error.");
    }

    switch (@typeInfo(Writer.Error)) {
        .ErrorSet => {},
        else => @compileError(message ++ " Cannot use " ++ @typeName(Writer) ++ " because its member Error is not an error set."),
    }

    if (@TypeOf(Writer.write) != fn (Writer, []const u8) Writer.Error!usize) {
        @compileError(message ++ " Cannot use " ++ @typeName(Writer) ++ " because its member write has type " ++ @typeName(@TypeOf(Writer.write)) ++ ". Expected " ++ @typeName(fn (Writer, []const u8) Writer.Error!usize) ++ ".");
    }
}

pub fn toBMP(self: Self, bmp_writer: anytype) @TypeOf(bmp_writer).Error!void {
    verifyWriter(@TypeOf(bmp_writer));
